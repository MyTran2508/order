...
import { IGiftCard, ICartItem } from '@/types'
...
export function GiftCardSelectedDrawer({
  ...
  const handleAddToCart = () => {
    const cartItem: ICartItem = {
      ...
      slug: `gift_card_${Date.now()}`,
      type: 'gift-card',
      orderItems: [
        {
          id: `gift_card_item_${Date.now()}`,
    ...
    if (onAddToCart) {
      onAddToCart(selectedCard, quantity)
    }
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-grow">
                <div
                  className="max-h-16 overflow-auto text-lg font-medium"
                  title={selectedCard.title}
                >
                  {selectedCard.title}
                </div>
                <div
                  className="mt-1 max-h-24 overflow-auto whitespace-normal break-words text-sm text-gray-600"
